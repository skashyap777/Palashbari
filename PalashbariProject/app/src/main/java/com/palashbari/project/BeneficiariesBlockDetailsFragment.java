...
public class BeneficiariesBlockDetailsFragment extends Fragment implements ConnectivityReceiver.ConnectivityListener{

    View view;
    String profile_image_uri,schemeName,schemeId;
    UserManagement userManagement;
    private ConnectivityReceiver connectivityReceiver;
    boolean internetResult = false;
    LinearLayout lin,noRec,paginationLayout,itemContainer;
    ProgressBar spin_kit;
    TextView beneficiariesTextView,beneficiariesNameTextView;
    ...
    private TextView selectedBlockName = null;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        view = inflater.inflate(R.layout.fragment_beneficiaries_block_details, container, false);
    ...
    public void onConnectivityChanged(boolean isConnected) {
        if (isConnected) {
            ...
            if (!isAPICallRunning) {
                ...
                AsyncTaskBeneficiariesBlockDetails asyncTaskBeneficiariesBlockDetails = new AsyncTaskBeneficiariesBlockDetails(parameter);
                asyncTaskBeneficiariesBlockDetails.execute();
    ...
    public class AsyncTaskBeneficiariesBlockDetails extends AsyncTask<Void, Void, Void> {
        Map<String, String> parameter;

        public AsyncTaskBeneficiariesBlockDetails(Map<String, String> parameter) {
            this.parameter = parameter;
        }

        @Override
        protected Void doInBackground(Void... voids) {
            JsonAPICall.makePostRequestWithTokenAndFormData(getActivity(),  API.block_data, parameter, new JsonAPICall.VolleyCallback() {
                @Override
    ...
    private void processValue(JSONArray jsonArray) throws JSONException {
        for (int k = 0; k < jsonArray.length(); k++) {
            ...
            if (getActivity() != null) {
                ...
                TextView nameTextview = mainLayout.findViewById(R.id.mandalNameTextview);
                ...
                mainCardLayout.setOnClickListener(v -> {
                    if (selectedCard != null && selectedBlockName != null && selectedImageView != null) {
                        selectedCard.setBackgroundColor(ContextCompat.getColor(getActivity(), R.color.grey3));
                        selectedBlockName.setTextColor(ContextCompat.getColor(getActivity(), R.color.black));
                    ...
                    selectedBlockName = nameTextview;

                    Fragment newFragment = new BeneficiariesDetailsFragment();
                    Bundle bundle = new Bundle();
                    bundle.putString("schemeName", schemeName);
                    bundle.putString("schemeId", schemeId);
                    bundle.putString("blockName", name);
                    bundle.putString("blockId", String.valueOf(id));
