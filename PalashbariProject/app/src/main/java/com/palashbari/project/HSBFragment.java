...
public class HSBFragment extends Fragment implements ConnectivityReceiver.ConnectivityListener {
    ...
    LinearLayout lin, noRec, itemContainer,paginationLayout,panchayatLayout,filteredWardLayout,filteredGpLayout;
    ProgressBar spin_kit;
    JSONArray gaoArray,wardArray;
    ImageView filter;
    Dialog filterDialog;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        filteredWardLayout = view.findViewById(R.id.filteredWardLayout);
        filteredGpLayout = view.findViewById(R.id.filteredGpLayout);
        filteredGp = view.findViewById(R.id.filteredGp);
        filteredWard = view.findViewById(R.id.filteredWard);

        filterDialog = new Dialog(requireContext());
        filterDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        filterDialog.setContentView(R.layout.layout_bottom_sheet_hsb);
        ...
        CardView searchWard = filterDialog.findViewById(R.id.searchWard);
        EditText searchEditTextPanchayat = filterDialog.findViewById(R.id.searchEditTextPanchayat);
        EditText searchEditTextWard = filterDialog.findViewById(R.id.searchEditTextWard);
        ...
        filter.setOnClickListener(view -> {
            ...
            tempSelectedWard = selectedWard;
            tempSelectedPanchayatName = selectedPanchayatName;
            tempSelectedWardName = selectedWardName;

            if (tempSelectedPanchayat != null && internetResult) {
                new AsyncTaskWard(tempSelectedPanchayat).execute();
            }

            panchayatLayout.setOnClickListener(v -> {
                itemContainerWard.removeAllViews();
                itemContainerGaonPanchayat.removeAllViews();

                noRecWard.setVisibility(View.GONE);
                searchWard.setVisibility(View.GONE);

                itemContainerWard.setVisibility(View.GONE);
                itemContainerGaonPanchayat.setVisibility(VISIBLE);

                searchEditTextPanchayat.setText("");
                searchEditTextWard.setText("");
                searchEditTextPanchayat.clearFocus();
                searchEditTextWard.clearFocus();
                ...
                wardLayout.setBackground(null);
                wardText.setTextColor(getResources().getColor(R.color.grey2));

                if (gaoArray != null && gaoArray.length() > 0) {
                    try {
                        searchPanchayat.setVisibility(VISIBLE);
                        showDialog(gaoArray, "panchayat", itemContainerGaonPanchayat, clear, showResult, filterDialog);
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                } else {
                    noRecGaonPanchayat.setVisibility(VISIBLE);
                    searchPanchayat.setVisibility(View.GONE);
                }
            });


            wardLayout.setOnClickListener(v -> {
                itemContainerWard.removeAllViews();
                itemContainerGaonPanchayat.removeAllViews();
                ...
                itemContainerWard.setVisibility(VISIBLE);
                ...
                wardLayout.setBackground(getResources().getDrawable(R.drawable.active_button));
                wardText.setTextColor(getResources().getColor(R.color.green));
                panchayatLayout.setBackground(null);
                panchayatText.setTextColor(getResources().getColor(R.color.grey2));

                if (wardArray != null && wardArray.length() > 0) {
                    try {
                        searchWard.setVisibility(VISIBLE);
                        showDialog(wardArray, "ward", itemContainerWard, clear, showResult, filterDialog);
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                } else {
                    noRecWard.setVisibility(VISIBLE);
                    searchWard.setVisibility(View.GONE);
                }
            });
            ...
            filterDialog.setOnDismissListener(dialogInterface -> {
                ...
                tempSelectedWard = null;
                tempSelectedPanchayatName = null;
                tempSelectedWardName = null;
                wardArray = null;
            });
        });
        ...
        searchEditTextWard.addTextChangedListener(new TextWatcher() {
            @Override
            public void beforeTextChanged(CharSequence s, int start, int count, int after) {}
                ...
                try {
                    ...
                    if (wardArray != null && wardArray.length() > 0) {
                        for (int i = 0; i < wardArray.length(); i++) {
                            JSONObject obj = wardArray.getJSONObject(i);
                            if (obj.getString("name").toLowerCase().contains(text.toLowerCase())) {
                                filteredArray.put(obj);
                            }
                        }
                        ...
                        if (filteredArray.length() > 0) {
                            showDialog(filteredArray, "ward", itemContainerWard, clear, showResult, filterDialog);
                        }
                    }
    ...
    private void showDialog(JSONArray jsonArray, String type, FlexboxLayout itemLayout, CardView clear, CardView showResult, Dialog dialog) throws JSONException {
        ...
        // --- Ward List Handling ---
            ...
            for (JSONObject obj : jsonObjects) {
                int id = obj.getInt("id");
                if (tempSelectedWard != null && tempSelectedWard.equals(String.valueOf(id))) {
                    selectedObject = obj;
                } else {
                    otherObjects.add(obj);
                }
            ...
            for (int index = 0; index < finalList.size(); index++) {
                JSONObject json = finalList.get(index);
                String wardName = json.getString("name");
                int id = json.getInt("id");
                ...
                if (tempSelectedWard != null && tempSelectedWard.equals(String.valueOf(id))) {
                    System.out.println("here: "+tempSelectedWard+String.valueOf(id));
                    radio.setChecked(true);
                }
                ...
                mainLayout.setOnClickListener(v -> {
                    ...
                    radio.setChecked(true);
                    tempSelectedWard = String.valueOf(id);
                    tempSelectedWardName = wardName;
                });
        ...
        showResult.setOnClickListener(v -> {
            ...
            selectedWard = tempSelectedWard;
            selectedPanchayatName = tempSelectedPanchayatName;
            selectedWardName = tempSelectedWardName;
            selectedPage = 0;
            selectedPageButton = null;
            dialog.dismiss();
    ...
    public void onConnectivityChanged(boolean isConnected) {
        if (isConnected) {
            ...
            if (!isAPICallRunning) {
                ...
                parameter.put("ward_number", "");
                AsyncTaskHSB asyncTaskHSB = new AsyncTaskHSB(parameter);
                asyncTaskHSB.execute();
    ...
    public class AsyncTaskWard extends AsyncTask<Void, Void, Void> {
        String gaonPanchayatId;

        public AsyncTaskWard(String gaonPanchayatId) {
        ...
        protected Void doInBackground(Void... voids) {
            ...
            JsonAPICall.makePostRequestWithToken(getActivity(), API.ward,jsonObject , new JsonAPICall.VolleyCallback() {
                @Override
                public void onSuccess(String response) {
        ...
        private void handleWardResponse(String response) throws JSONException {
            if (response != null) {
                ...
                if (jsonObject.has("status") && jsonObject.getInt("status") == 1) {
                    wardArray = jsonObject.getJSONArray("data");
                }
                if (itemContainerWard.getVisibility() == VISIBLE) {
                    itemContainerWard.removeAllViews();
                    try {
                        showDialog(wardArray, "ward", itemContainerWard, clear, showResult, filterDialog);
                    } catch (JSONException e) {
    ...
    public class AsyncTaskHSB extends AsyncTask<Void, Void, Void> {
        Map<String, String> parameter;

        public AsyncTaskHSB(Map<String, String> parameter) {
            this.parameter = parameter;
        }

        @Override
        protected Void doInBackground(Void... voids) {
            JsonAPICall.makePostRequestWithTokenAndFormData(getActivity(),  API.hsb_data, parameter, new JsonAPICall.VolleyCallback() {
                @Override
                public void onSuccess(String response) {
                    try {
                        handleResponse(response);
    ...
    private void processValue(JSONArray jsonArray) throws JSONException {
        for (int k = 0; k < jsonArray.length(); k++) {
            ...
            String wardNumber = json.getString("ward_names");
            String panchayat = json.getString("gaon_panchayat_names");

            if (getActivity() != null) {
                View mainLayout = LayoutInflater.from(getActivity()).inflate(R.layout.hsb_item, itemContainer, false);

                TextView nameTextview = mainLayout.findViewById(R.id.nameTextview);
                TextView numberTextview = mainLayout.findViewById(R.id.numberTextview);
                TextView wardTextview = mainLayout.findViewById(R.id.wardTextview);
                ...
                wardTextview.setText(wardNumber);
    ...
    private void fetchData(int start, int length, String searchText) {
        ...
        parameter.put("ward_number", selectedWard != null ? selectedWard : "");

        requireActivity().runOnUiThread(() -> {
            if (!TextUtils.isEmpty(selectedPanchayat) || !TextUtils.isEmpty(selectedWard)) {
            ...
            if (!TextUtils.isEmpty(selectedWard)) {
                filteredWardLayout.setVisibility(View.VISIBLE);
                filteredWard.setText(selectedWardName);
            } else {
                filteredWardLayout.setVisibility(View.GONE);
            }
        });
