...
public class BeneficiariesFragment extends Fragment implements ConnectivityReceiver.ConnectivityListener{
    ...
    private LinearLayout selectedCountLayout = null;
    private TextView selectedBenName = null;
    ...
    public void onConnectivityChanged(boolean isConnected) {
        if (isConnected) {
            ...
            if (!isAPICallRunning) {
                ...
                AsyncTaskScheme asyncTaskScheme = new AsyncTaskScheme(parameter);
                asyncTaskScheme.execute();
    ...
    public class AsyncTaskScheme extends AsyncTask<Void, Void, Void> {
        Map<String, String> parameter;

        public AsyncTaskScheme(Map<String, String> parameter) {
            this.parameter = parameter;
        }

        @Override
        protected Void doInBackground(Void... voids) {
            JsonAPICall.makePostRequestWithTokenAndFormData(getActivity(),  API.scheme_data, parameter, new JsonAPICall.VolleyCallback() {
                @Override
    ...
    private void processValue(JSONArray jsonArray) throws JSONException {
        for (int k = 0; k < jsonArray.length(); k++) {
            ...
            String schemeName = json.getString("name");
            int schemeId = json.getInt("id");
            String count = json.getString("member_count");

            if (getActivity() != null) {

                View mainLayout = LayoutInflater.from(getActivity()).inflate(R.layout.beneficiaries_item, itemContainer, false);

                TextView benNameTextview = mainLayout.findViewById(R.id.benNameTextview);
                TextView countTextView = mainLayout.findViewById(R.id.countTextView);
                LinearLayout countLayout = mainLayout.findViewById(R.id.countLayout);
                LinearLayout mainCardLayout = mainLayout.findViewById(R.id.mainCardLayout);

                benNameTextview.setText(schemeName);
                countTextView.setText(count);

                mainCardLayout.setBackgroundColor(ContextCompat.getColor(getActivity(), R.color.grey3));
                countLayout.setBackgroundColor(ContextCompat.getColor(getActivity(), R.color.green));
                benNameTextview.setTextColor(ContextCompat.getColor(getActivity(), R.color.black));

                mainCardLayout.setOnClickListener(v -> {
                    if (selectedCard != null && selectedCountLayout != null && selectedBenName != null) {
                        selectedCard.setBackgroundColor(ContextCompat.getColor(getActivity(), R.color.grey3));
                        selectedCountLayout.setBackgroundColor(ContextCompat.getColor(getActivity(), R.color.green));
                        selectedBenName.setTextColor(ContextCompat.getColor(getActivity(), R.color.black));
                    }

                    benNameTextview.setTextColor(ContextCompat.getColor(getActivity(), R.color.white));
                    mainCardLayout.setBackgroundColor(ContextCompat.getColor(getActivity(), R.color.green));
                    countLayout.setBackgroundColor(ContextCompat.getColor(getActivity(), R.color.green1));

                    selectedCard = mainCardLayout;
                    selectedCountLayout = countLayout;
                    selectedBenName = benNameTextview;

                    Fragment newFragment = new BeneficiariesBlockDetailsFragment();
                    Bundle bundle = new Bundle();
                    bundle.putString("schemeName", schemeName);
                    bundle.putString("schemeId", String.valueOf(schemeId));
